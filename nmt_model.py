...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        ###
        # Step 1: Construct Tensor `X` of source sentences
        ...
        # Step 3: Compute `enc_hiddens`, `last_hidden`, `last_cell`
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###             over. Otherwise, you will remove the batch dimension accidentally, if batch_size = 1.
        ###
        # Step 1: Apply the decoder to `Ybar_t` and `dec_state` to obtain the new dec_state
        dec_state = self.decoder(Ybar_t, dec_state)

        # Step 2: Split dec_state into its two parts (dec_hidden, dec_cell)
        dec_hidden, dec_cell = dec_state

        # Step 3: Compute the attention scores e_t
        ...
        ###
        # Step 4: Apply softmax to e_t to yield alpha_t
        alpha_t = F.softmax(e_t, dim=1)

        # Step 5: Use batched matrix multiplication between alpha_t and enc_hiddens to obtain the attention output vector
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)

        # Step 6: Concatenate dec_hidden with a_t to compute tensor U_t
        U_t = torch.cat([a_t, dec_hidden], dim=1)

        # Step 7: Apply the combined output projection layer to U_t to compute tensor V_t
        V_t = self.combined_output_projection(U_t)

        # Step 8: Compute tensor O_t by first applying the Tanh function and then the dropout layer
